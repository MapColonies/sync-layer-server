...
- **`config/default.json`** - new `db` section (host, port, database, username, password, ssl, poolSize).
...
| `src/dal/repositories/syncStateRepository.ts` | in-memory → TypeORM, async |
...
2. Populate the `db` section of `config/default.json` (or the environment-specific config file) with the target host, port, database, username, password, ssl, and poolSize.
